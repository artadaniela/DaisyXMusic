...
async def play(_, message: Message):
    ...
    if audio:
        ...
        try:
            toxxt = "**Pilih dulu lagu yg ingin lu putar**\n\n"
            j = 0
            useer=user_name
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(text="👑 King", url=f"t.me/lifeinsinn"),
                        InlineKeyboardButton(text="Download 📥", url=f"t.me/XXXsquad_18plus"),
                    ],
                    [InlineKeyboardButton(text="❌ Close", callback_data="cls")],
                ]
            )
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer("Bukan lu yang req lagu ini bego!!!", show_alert=True)
        return
    await cb.message.edit("Sabar tod... Ngocok dulu")
    x=int(x)
    try:
